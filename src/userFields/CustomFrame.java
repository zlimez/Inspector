...
public class CustomFrame {
	private Map<String, Value> fields; // each frame should hold the state of the field pool at the frame
	 ...
	 */
	public CustomFrame(final int numLocals, final int numStack) {
	 ...
	 * @return this frame.
	 */
	public CustomFrame(final CustomFrame frame) {
		this(frame.numLocals, frame.values.length - frame.numLocals);
		fields = frame.fields;
		returnValue = frame.returnValue;
		numStack = frame.numStack;
		System.arraycopy(frame.values, 0, values, 0, numLocals + numStack);
	}

	
	public CustomFrame init(final CustomFrame frame) {
		returnValue = frame.returnValue;
		numStack = frame.numStack;
		List<Value> existingValues = new ArrayList<Value>(); 
		List<Value> newValues = new ArrayList<Value>();
		for (int i = 0; i < numLocals + numStack; i++) { 
			values[i] = CustomFrame.copyStack(existingValues, newValues, frame.values[i]);
		}
		fields = new HashMap<>();
		frame.fields.forEach((field, val) -> {
			fields.put(field, CustomFrame.copyStack(existingValues, newValues, val));
		});
		return this;
	}
	 ...
	 */
	public Value getStack(final int index) {
	 ...
	 */
	public void setStack(final int index, final Value value) {
	 ...
	 */
	public void execute(final AbstractInsnNode insn, final CustomInterpreter interpreter) throws AnalyzerException {
		Value value1;
		Value value2;
		Value value3;
		Value value4;
		int var;
		interpreter.setUserControlledFields(fields);
	...
	private boolean executeDupX2(final AbstractInsnNode insn, final Value value1, final CustomInterpreter interpreter) throws AnalyzerException {
		Value value2 = pop();
		if (value2.getSize() == 1) {
			Value value3 = pop();
			if (value3.getSize() == 1) {
	...
	private void executeInvokeInsn(final AbstractInsnNode insn, final String methodDescriptor, final CustomInterpreter interpreter) throws AnalyzerException {
	  ...
	  public String toString() {
		  ...
		  for (int i = 0; i < getStackSize(); ++i) {
			  stringBuilder.append(getStack(i) + "\n");
		  }
		  stringBuilder.append("\n");
	...
	public boolean merge(final CustomFrame frame, final CustomInterpreter interpreter) throws AnalyzerException {
		if (frame.getStackSize() != this.getStackSize()) {
			throw new AnalyzerException(null, "Incompatible stack heights");
		}
		boolean changed = false;
		if (this.fields.equals(frame.fields)) {
		} else 
			changed = true;
		this.fields = frame.fields; // convenience sake to create a new Map<> fields for each frame from currentFrame
		for (int i = 0; i < numLocals + numStack; i++) {
 			Value val = interpreter.merge(values[i], frame.values[i]);
	...
	public boolean mergeJumpFrame(final CustomFrame frame, final Interpreter<Value> interpreter) throws AnalyzerException {
		...
		for (int i = 0; i < numLocals + numStack; i++) { 
			...
			values[i] = CustomFrame.copyStack(existingValues, newValues, val);
		}
		fields = new HashMap<>();
		((CustomInterpreter) interpreter).getUserControlledFieldsForFrame().forEach((field, val) -> {
			fields.put(field, CustomFrame.copyStack(existingValues, newValues, val));
		});
		return changed;
	}
	
	public static Value copyStack(List<Value> existingValues, List<Value> newValues, Value value) {
		if (value instanceof ReferenceValue || value instanceof MultiDArray || value instanceof ArrayValue) {
			if (existingValues.size() > 0) {
				for (int i = 0; i < existingValues.size(); i++) {
					if (existingValues.get(i) == value) {
			...
			if (value instanceof ReferenceValue) {
				newValue = ((ReferenceValue) value).newRef();
			} else if (value instanceof MultiDArray) {
				newValue = ((MultiDArray) value).newMultiArr();
			} else {
				newValue = ((ArrayValue) value).newArr();
			} 
